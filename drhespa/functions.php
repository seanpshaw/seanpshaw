...
function drhespa_scripts() {
  wp_register_script( 'easing', get_template_directory_uri() . '/js/jquery.easing.1.3.js', array( 'jquery' ), 'version', true );
  wp_register_script( 'orbit', get_stylesheet_directory_uri() . '/inc/js/orbit.js', array( 'jquery' ), 'version', true );
  wp_register_script( 'jcarousel', get_stylesheet_directory_uri() . '/inc/js/jcarousel.simple.js', array( 'jquery' ), 'version', true );
  wp_register_script( 'fredcarousel', get_stylesheet_directory_uri() . '/inc/js/fredcarousel.js', array( 'jquery' ), 'version', true );
  wp_register_script( 'mobilemenu', get_stylesheet_directory_uri() . '/inc/js/mobilemenu.js', array( 'jquery' ), 'version', true );
  wp_register_script( 'custom-scripts', get_stylesheet_directory_uri() . '/inc/js/script.js', array( 'jquery' ), 'version', true );

  wp_enqueue_script( 'easing' );
  wp_enqueue_script( 'orbit' );
  wp_enqueue_script( 'jcarousel' );
  wp_enqueue_script( 'fredcarousel' );
  wp_enqueue_script( 'mobilemenu' );
  wp_enqueue_script( 'custom-scripts' );

}

add_action( 'wp_enqueue_scripts', 'drhespa_scripts' );
...
function SliderContent(){
    ...
    $args = array( 'post_type' => 'Orbit', 'posts_per_page' => 5, 'order' => 'DESC');
    ...
    while ( $loop->have_posts() ) : $loop->the_post();
      ...
      if(has_post_thumbnail()) {
        ...
        echo '<span class="orbit-caption" id="htmlCaption">';
...
function themeit_tiny_mce_before_init( $settings ) {
  ...
  $style_formats = array(
      ...
      array( 'title' => 'float', 'block' => 'div',  'classes' => 'float' )
  );
